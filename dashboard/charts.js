...
function renderHumanGateCards(humanGates) {
  ...
  var rejected = humanGates.filter(function(hg) { return hg.status === 'rejected'; });
  if (pending.length === 0 && rejected.length === 0) { container.innerHTML = ''; return; }

  var html = '';
  if (pending.length > 0) {
    html += '<div style="font-size:13px;font-weight:600;margin-bottom:8px;color:#fbbf24">Pending Approvals</div>';
    pending.forEach(function(hg) { html += buildHumanGateCardHtml(hg); });
  }
  if (rejected.length > 0) {
    html += '<div style="font-size:13px;font-weight:600;margin:12px 0 8px;color:#ef4444">Rejected Gates</div>';
    rejected.forEach(function(hg) { html += buildHumanGateCardHtml(hg); });
  }
  container.innerHTML = html;
}
...
function buildHumanGateCardHtml(hg) {
  ...
  var borderColor = isRejected ? '#ef4444' : '#fbbf24';
  var dotColor = isRejected ? '#ef4444' : '#fbbf24';
  var dotAnim = isRejected ? '' : 'animation:pulse 1.5s infinite;';
  var buttons = '';
  if (isRejected) {
    buttons = '<button class="gate-retry-btn" data-key="' + escAttr(key) + '" style="font-size:11px;padding:2px 8px;border:1px solid #fbbf24;background:transparent;color:#fbbf24;border-radius:4px;cursor:pointer">Retry</button>';
  } else {
    buttons = '<button class="gate-cancel-btn" data-key="' + escAttr(key) + '" style="font-size:11px;padding:2px 8px;border:1px solid #ef4444;background:transparent;color:#ef4444;border-radius:4px;cursor:pointer">Cancel</button>';
  }
  return '<div id="hg-card-' + escAttr(key) + '" style="background:var(--surface);border:1px solid ' + borderColor + ';border-radius:6px;padding:10px;margin-bottom:6px">' +
    '<div style="display:flex;align-items:center;gap:8px;margin-bottom:4px">' +
    '<span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:' + dotColor + ';' + dotAnim + '"></span>' +
    '<strong style="font-size:12px">' + esc(stepId) + '</strong>' +
    '<span class="badge ' + (isRejected ? 'badge-err' : 'badge-warn') + '" style="font-size:10px">' + esc(subtype) + '</span>' +
    (isRejected ? '<span class="badge badge-err" style="font-size:10px">rejected</span>' : '') +
    '</div>' +
    ...
    '<div style="margin-top:6px;text-align:right">' + buttons + '</div>' +
    '</div>';
}
...
// Event delegation for human gate retry buttons.
document.addEventListener('click', function(e) {
  if (!e.target.classList.contains('gate-retry-btn')) return;
  var key = e.target.dataset.key;
  if (!key) return;
  if (!confirm('Retry rejected gate ' + key + '?')) return;
  e.target.disabled = true;
  e.target.textContent = 'Retrying...';
  fetch('/api/human-gates/' + encodeURIComponent(key) + '/retry', {
    method: 'POST',
    headers: {'Content-Type': 'application/json'},
    body: JSON.stringify({})
  }).then(function(res) {
